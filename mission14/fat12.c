...
--*/
{
	...
	ULONG SPLITFLAG;							// 分割标志
	ULONG NumOfFrontBuffer = BytesToWrite;		// 前一半需要写入的字节
	ULONG NumOfLatterBuffer = 0;				// 后一半需要写入的字节
	...
	//否则要分成两半
		...
		SPLITFLAG = 1;
		NumOfFrontBuffer = 512 - BytesNumOfCluster;
		NumOfLatterBuffer = (BytesToWrite - NumOfFrontBuffer) % 512;
		SpanOfSector = (BytesToWrite - NumOfFrontBuffer)/512;
	}	
	...
	if(SPLITFLAG == 0){
		...
		//划分缓存区
		FrontBuffer = Buffer;
		MidBuffer = &Buffer[NumOfFrontBuffer];
		LatterBuffer = &Buffer[NumOfFrontBuffer + SpanOfSector * 512];
	}
	/*************************************************************************/	
	...
	/****************写入尾扇区************************/
	if(SPLITFLAG == 1 && NumOfLatterBuffer > 0){
		// 顺着簇链向后查找写入的起始位置所在簇的簇号。
		PrevClusterNum = 0;
		CurrentClusterNum = File->FirstCluster;
		// 根据簇的大小，计算写入的起始位置在簇链的第几个簇中（从 0 开始计数）
		...
		// 如果文件长度增加了则必须修改文件的长度。
		if (Offset + NumOfLatterBuffer > File->FileSize) {
			File->FileSize = Offset + BytesToWrite;
