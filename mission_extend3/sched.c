...
--*/
{
	...
	// 在此添加代码，实现时间片轮转调度算法。
	//
	// 只有当前线程是运行状态才进行时间片调度算法
	// 如果当前线程被阻塞，则不用
	if(NULL != PspCurrentThread && Running == PspCurrentThread->State){
		
		PspCurrentThread->RemainderTicks--;	// 当前运行线程的剩余时间片减1
		PspCurrentThread->UsedTicks++;		// 当前运行线程已使用的时间片加1
		
		// 已使用的时间片等于生命周期，则结束线程
		if(PspCurrentThread->UsedTicks == PspCurrentThread->LifeTimeTicks){
		...
		/**
			如果时间片为0，执行两个操作
			1、重新为当前线程分配时间片
			2、如果存在优先级相同的线程并且处于就绪状态，则将当前线程的状态变为就绪状态
		**/
		if(0 == PspCurrentThread->RemainderTicks){
			PspCurrentThread->RemainderTicks = TICKS_OF_TIME_SLICE;
			
			/******多级反馈队列调度算法实现核心部分，若不使用该方法则注释掉********/
			if(PspCurrentThread->Priority > 0){
				PspCurrentThread->Priority--;							// 提高优先级
				PspCurrentThread->InitTicks += TICKS_OF_TIME_SLICE;		// 初始时间片增加一个单位
				PspCurrentThread->RemainderTicks = PspCurrentThread->InitTicks;
				PspThreadSchedule();
			}
			/**********************************************************************/
			
			// 判断就绪队列中是否存在优先级相同的线程，有就执行操作
